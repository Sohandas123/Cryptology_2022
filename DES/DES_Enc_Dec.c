...
unsigned char *C, *D;
unsigned char *plaintxt, *ciphertxt;

void Apply_Per_PI(unsigned char *, unsigned char *, int **, int, int);
void Expansion(unsigned char *, unsigned char *, int **);
...
void rightshift(unsigned char *, unsigned char *);
void join_CD(unsigned char *, unsigned char *, unsigned char *);

void round_Key(int);
void Rev_round_key(int);
void Feistel(int);
int v;
void print(int);  // Decoration part
void DES(unsigned char *, unsigned char key[8], unsigned char *, int);
void Dec_DES(unsigned char *, unsigned char key[8], unsigned char *);
...
{
    int i, j;
    plaintxt = (unsigned char *)calloc(8, sizeof(unsigned char));
    // unsigned char Ptxt[8] = "SohanDas";
    unsigned char Ptxt[8] = {'S', 'o', 'h', 'a', 'n', 'D', 'a', 's'};
    for (i = 0; i < 8; i++)
        plaintxt[i] = Ptxt[i];
    char testkey[8] = "ThankGod";
    ciphertxt = (unsigned char *)calloc(8, sizeof(unsigned char));
    

    printf("\n Encryption :\n");
    DES(plaintxt, testkey, ciphertxt, 0);
    print(0); // Encryption

    printf("\n-------------------------------------------------------------\n");
    printf("\n Decryption :\n");
    Dec_DES(ciphertxt, testkey, plaintxt);
    print(1);  // decryption
...
void trans56to28(unsigned char *input56, unsigned char *out28C, unsigned char *out28D)
{
    ...
    // printf("\n\t\t\t C[0]    D[0]");     /* for matching with C[16] and D[16]  in round_Key() */
    // for(i=0; i<7; i++)
    // {
    //     printf("\n\t\t\t  %x      %x", C[i],D[i]);
    // }
...
void rightshift(unsigned char *C, unsigned char *D)
{
//    printf("\n right-shift invoked    ");
    unsigned char new_bc, old_bc, new_bd, old_bd;
    int i;
    old_bc = (C[6] & 1) << 3;
    old_bd = (D[6] & 1) << 3;
    for (i = 0; i < 7; i++)
...
void round_Key(int t)
{
    ...
    // if(t==16){              /* for matching with C[0] and D[0] in trans56to28() */
    //     printf("\n\t\t\t C[16]   D[16]");
    //     for(i=0; i<7; i++)
    //         printf("\n\t\t\t  %x      %x", C[i],D[i]);
    // }
...
void Rev_round_key(int t)
{
    ...
    /* Right shifting by one bit */

    if ((t != 1) && (t != 8) && (t != 15)) //
        rightshift(C, D);                  /* Since for round 2,9,16 there is only one right shift in decDES */

    rightshift(C, D); /* At t-round key generation C[t-1] right-shifted to C[t] and D[t-1] right shifted to D[t] */
}
...
void DES(unsigned char *plantext, unsigned char key[8], unsigned char *cipertxt, int x)
{
    v = x;
    ...
    Apply_Per_PI(plantext, IP_Ptxt, IP, 8, 8); /* Applying IP on Plaintext */
    // for (i = 0; i<8; i++)
    //     printf("%d, ",IP_Ptxt[i]);
    // printf("\n");
    ...
    {
//        printf("\n------------------------\n Round : %d\n\n", i);
        if (v == 0) // for encryption
        {
            round_Key(i); // round key K[i] is generated and now it can be used
//            printf("\n Round key %d : ", i);
        }
        if (v == 1) // for decryption
        {
            Rev_round_key(i); // reverse round key K[i] is generated and now it can be used
//            printf("\n Reverse Round key %d : ", 17 - i);
        }
/*        for (j = 0; j < 6; j++)
            printf(" %x", K[i][j]);
        printf("\n");   */
    ...
    Apply_Per_PI(pre_cipher, cipertxt, InvIP, 8, 8);
}

void Dec_DES(unsigned char *cipertxt, unsigned char key[8], unsigned char *plantxt)
{
    for (int i = 0; i < 8; i++)
    {
        plantxt[i] = cipertxt[i];
        cipertxt[i] = 0x00;
    }
    DES(plantxt, key, cipertxt, 1); // Decrtyption
}
...
void print(int x)
{
    ...
    for (i = 0; i < 8; i += 1)
    {
        printf("  %x ", K[0][i]); /* Private Key Printing */
    }
    /* ----------------------------------------------- */
    if (x == 0) // Encryption
        printf("\n   Ciphertxt   :");
